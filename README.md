...
- Built-ins: `int(x)` / `float(x)` conversions, `len(x)`, `push(a, x)` (append), `has`/`delete`/`keys` (maps), `str(x)` (stringify), `chr(b)` (byte → 1-char string), `read_line()` (stdin → string / `null` at EOF)
...
- Control flow: `if/else`, `while`, `for (init; cond; step)`, `for (x in coll)` (arrays/maps), `break`, `continue` — conditions must be `bool`
