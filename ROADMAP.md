...
- [~] Begin the **standard library**: collections, strings, math, basic I/O. *(Arrays + `push`, math built-ins, an associative `map` type — [RFC 0002](docs/rfcs/0002-map-type.md), shipped in v0.24 — **nested collections** (`[[T]]`, `{string: [V]}`, v0.29), the string toolkit (`substr`/`split`/`join` + `int`/`float`/`is_int`/`is_float` parsing), and file I/O (`read_file`/`write_file`, v0.28) are done; more stdlib ongoing.)*
