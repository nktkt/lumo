...
//! 低レベル化する。各式の型は下から組み立て直して LLVM の型選択
//! (int=i64 / bool=i1) と print の書式選択に使う。
//!
...
impl<'ctx> CodeGen<'ctx> {
    ...
    fn llvm_ty(&self, ty: Type) -> inkwell::types::IntType<'ctx> {
        match ty {
            Type::Int => self.ctx.i64_type(),
    ...
    pub fn compile(&mut self, program: &Program) -> Result<(), Diagnostic> {
        ...
        for f in program {
            let param_types: Vec<BasicMetadataTypeEnum> =
                f.params.iter().map(|p| self.llvm_ty(p.ty).into()).collect();
            let fn_type = self.llvm_ty(f.ret).fn_type(&param_types, false);
    ...
    fn gen_function(&mut self, f: &Function) {
        ...
        for (i, p) in f.params.iter().enumerate() {
            ...
            let alloca = self
                .builder
                .build_alloca(self.llvm_ty(p.ty), &p.name)
                .unwrap();
        ...
        if self.block_open() {
            let zero = self.llvm_ty(f.ret).const_int(0, false);
    ...
    fn gen_expr(&mut self, e: &Expr) -> (IntValue<'ctx>, Type) {
        match &e.kind {
            ExprKind::Int(n) => (self.i64t().const_int(*n as u64, true), Type::Int),
            ExprKind::Bool(b) => (
            ...
            ExprKind::Unary { op, expr } => {
                let (v, _) = self.gen_expr(expr);
                match op {
                    UnOp::Neg => (self.builder.build_int_neg(v, "neg").unwrap(), Type::Int),
                    UnOp::Not => (self.builder.build_not(v, "not").unwrap(), Type::Bool),
                }
            }
            ExprKind::Binary { op, lhs, rhs } => match op {
                ...
                _ => {
                    ...
                    let b = &self.builder;
                    match op {
                        BinOp::Add => (b.build_int_add(l, r, "add").unwrap(), Type::Int),
                        BinOp::Sub => (b.build_int_sub(l, r, "sub").unwrap(), Type::Int),
                        BinOp::Mul => (b.build_int_mul(l, r, "mul").unwrap(), Type::Int),
                        BinOp::Div => (b.build_int_signed_div(l, r, "div").unwrap(), Type::Int),
                        BinOp::Mod => (b.build_int_signed_rem(l, r, "rem").unwrap(), Type::Int),
                        BinOp::Eq | BinOp::Ne | BinOp::Lt | BinOp::Le | BinOp::Gt | BinOp::Ge => {
                            ...
                            (b.build_int_compare(pred, l, r, "cmp").unwrap(), Type::Bool)
                        }
    ...
    fn gen_logical(&mut self, op: BinOp, lhs: &Expr, rhs: &Expr) -> (IntValue<'ctx>, Type) {
        let (l, _) = self.gen_expr(lhs);
        ...
        (phi.as_basic_value().into_int_value(), Type::Bool)
    }
